...
export default function App() {
  ...
  const [demographicsLoaded, setDemographicsLoaded] = useState(null); // {loaded, groups}
  ...
  const handleDemographicsUpload = async (file) => {
    setDemographicsFile(file);
    setError("");
    try {
      const formData = new FormData();
      formData.append("file", file);
      const res = await fetch(`${API_BASE}/upload-demographics`, {
        method: "POST",
        body: formData,
      });
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        throw new Error(err.detail || "Demographics upload failed");
      }
      const data = await res.json();
      setDemographicsLoaded(data);
    } catch (err) {
      setError(err.message || "Could not upload demographics CSV.");
      setDemographicsLoaded(null);
    }
  };
    ...
    try {
      ...
      const screenRes = await fetch(`${API_BASE}/screen`, {
        ...
        body: JSON.stringify({
          ...
          demographics: [], // demographics are matched server-side by filename via the CSV upload
        }),
      });
    ...
    <div className="app">
      ...
      <section className="input-section">
        ...
        <label className="field-label">
          Demographics CSV (optional — enables the fairness audit)
        </label>
        <input
          type="file"
          accept=".csv"
          onChange={(e) => e.target.files[0] && handleDemographicsUpload(e.target.files[0])}
        />
        <p className="file-count">
          {demographicsLoaded
            ? `Loaded ${demographicsLoaded.loaded} labels across groups: ${demographicsLoaded.groups.join(", ")}`
            : "CSV needs 'filename' and 'group' columns, e.g. resume1.pdf,Group A"}
        </p>
          ...
          <div className="fairness-column">
            ...
            {fairnessReport ? (
              ...
              <p className="fairness-empty-note">
                Upload a demographics CSV above before screening to see the fairness audit here.
              </p>
            )}
          </div>
        </section>
